...
const TableJadwalPembelajaran = (props: Props) => {
  const { onDay, onClass } = props;
  const { state } = useJadwalPembelajaranpageContext();
  const { timeReq, jadwalPembelajaranReq } = state;
      ...
      <table className="lg:w-[70%] w-full rounded table-auto">
        ...
        <tbody>
          {timeReq?.map((item, index) => {
            ...
            const matchedSchedule = jadwalPembelajaranReq.find(
              (s) =>
            ...
            return (
              <tr key={index} className="border-b-2 border-indigo-500">
                ...
                <td className="px-4 py-2  border-b-1">
                  ...
                  {item.type == "pembelajaran" && matchedSchedule?.pengajar && (
...
export default TableJadwalPembelajaran;
